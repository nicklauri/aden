...
impl Timer {
    ...
    pub fn elapsed(&self) -> Result<f64, String> {
        if !self.is_init {
            ...
            let time_elapsed: f64 =
                (elapsed.as_secs() * 1000) as f64 + elapsed.subsec_nanos() as f64 * 0.000001;
    ...
    pub fn elapsed_in_second(&self) -> Result<f64, String> {
        if !self.is_init {
            ...
            let elapsed: Duration = self.start.elapsed();
            let time_elapsed: f64 =
                elapsed.as_secs() as f64 + elapsed.subsec_nanos() as f64 * 0.000000001;
            Ok(time_elapsed)
        }
    }
...
pub fn get_root_path() -> String {
    // Now it returns current working directory
    std::env::current_dir()
        .unwrap()
        .to_str()
        .unwrap()
